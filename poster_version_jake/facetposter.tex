...
\baselineskip=3\baselineskip\Title{\bf Measuring Chaos}
\end{textblock}
...
\begin{center}{\Hugehead{\bf Andrew Wilson, Supervised by Dr. Anthony Quas}}
\end{center}
\end{textblock}
...
% Picture of a chaotic something or other
...
In order to study chaos, we need
models of the systems we would like to investigate. How can we abstract some of the previous examples in a way that can 
be studied mathematically?

\large{A {\bf cone} $C$ in $\R^k$ is a set such that for all $x,y \in \R^n$ \vspace{-8mm}
\begin{align*}
(1)\text{ } & x + y \in C \\
(2)\text{ } & ax \in C \text{ for all } a\in\R \text{ such that }a\ge0.
\end{align*}

\vspace{-8mm}
$0 \in C$ by condition (2). All subspaces of $\R^n$ are cones. In fact, cones are essentially subspaces of $\R$ in which only positive combination constants are considered. As such, if we are given some set $X$ in $\R^k$ then the cone $C_X$ generated by $X$ is the set of all linear combinations of vectors in $X$ using only positive coefficients, \

\vspace{-19mm} $$C = \{a_1x_1+...+a_nx_k | a_i \in \R^+\cup\{0\}, x_i \in X\}.$$}
\end{textblock}
...
\large{ All graphs on $n$ vertices with a fractional triangle decomposition will be in the cone generated by triangles in $\R^{{n \choose 2}}$. 
}
\end{textblock}
...
\LARGEhead{Investigation of the triangle cone}
\end{textblock}

\begin{textblock}{8}(8.4,2.25)
\large{
\begin{defn}
A {\bf facet} of a cone $C$ is a subset cone $K$ with the properties that 
